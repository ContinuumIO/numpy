...
When getting a view of a NumPy array, the base object is now set to 
the first array object that owns its own data (or the first sub-type 
for sub-types of the NumPy array).  As a result, chains of base objects
cannot be set-up (except for chains of different sub-types).  
